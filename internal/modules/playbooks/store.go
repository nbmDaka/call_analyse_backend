...
// List returns all playbooks belonging to the specified workspace.
func (s *PostgresStore) List(ctx context.Context, workspaceID uuid.UUID) ([]Playbook, error) {
	rows, err := s.pool.Query(ctx, `
SELECT id, workspace_id, name, COALESCE(description, ''), is_default, criteria, created_at, updated_at
FROM playbooks
...
ORDER BY is_default DESC, created_at ASC`, workspaceID)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.ID, &p.WorkspaceID, &p.Name, &p.Description, &p.IsDefault, &criteriaJSON, &p.CreatedAt, &p.UpdatedAt); err != nil {
...
// GetByID returns one playbook scoped to the specified workspace.
...
WHERE workspace_id = $1 AND id = $2`, workspaceID, id).Scan(
		&p.ID, &p.WorkspaceID, &p.Name, &p.Description, &p.IsDefault, &criteriaJSON, &p.CreatedAt, &p.UpdatedAt,
	)
...
// GetDefault returns the default playbook for the workspace or falls back to the earliest created.
...
// Create inserts a new playbook. If is_default is true, previous default is unset.
func (s *PostgresStore) Create(ctx context.Context, playbook Playbook) (Playbook, error) {
	...
	if playbook.IsDefault {
		if _, err := tx.Exec(ctx, `UPDATE playbooks SET is_default = FALSE WHERE workspace_id = $1`, playbook.WorkspaceID); err != nil {
...
VALUES ($1, $2, $3, $4, $5)
RETURNING id, workspace_id, name, COALESCE(description, ''), is_default, criteria, created_at, updated_at`,
		playbook.WorkspaceID, playbook.Name, playbook.Description, playbook.IsDefault, criteriaJSON,
	).Scan(&created.ID, &created.WorkspaceID, &created.Name, &created.Description, &created.IsDefault, &scannedCriteria, &created.CreatedAt, &created.UpdatedAt)
	if err != nil {
...
func (s *PostgresStore) Update(ctx context.Context, playbook Playbook) (Playbook, error) {
	...
	if playbook.IsDefault {
		if _, err := tx.Exec(ctx, `UPDATE playbooks SET is_default = FALSE WHERE workspace_id = $1 AND id <> $2`, playbook.WorkspaceID, playbook.ID); err != nil {
			return Playbook{}, fmt.Errorf("unset existing default: %w", err)
		}
	}
...
RETURNING id, workspace_id, name, COALESCE(description, ''), is_default, criteria, created_at, updated_at`,
	...
	).Scan(&updated.ID, &updated.WorkspaceID, &updated.Name, &updated.Description, &updated.IsDefault, &scannedCriteria, &updated.CreatedAt, &updated.UpdatedAt)
	if errors.Is(err, pgx.ErrNoRows) {
		return Playbook{}, ErrPlaybookNotFound
	}
	if err != nil {
...
func (s *PostgresStore) Delete(ctx context.Context, workspaceID, id uuid.UUID) error {
	cmd, err := s.pool.Exec(ctx, `DELETE FROM playbooks WHERE workspace_id = $1 AND id = $2`, workspaceID, id)
	if err != nil {
